...
Saiorm is a simple lightweight library for accessing database.It will take you have a easy way to use database,including SQL and NoSQL.
...
- Methods special to SQL databases:

    In SQL databases you can call method **execute** and method **executemany** to execute you own SQL.
...
- last query:
    ...
    **SQL Server** and **MongoDB** returns is empty string by default,if you want to get it,please pass **return_query=True** when call method **connect**.

    **SQL Server** required pymmsql do not return it, it's generated by saiorm via format query string with params, maybe not real.
...
- MongoDB is not full support:
    ...
    Support select,get,update,insert,insert_many,delete,increase,decrease,where,limit,order_by
